...
from utils.trans_tools import soft_update, huber_loss, mse_loss, _n2t
import numpy as np
from utils.popart import PopArt


class M_QMix:#for train and update QMIX 
    def __init__(self, args, num_agents, policies, policy_mapping_fn, device=torch.device("cuda:0"), vdn=False):
        ...
        self.opti_eps = float(self.args.opti_eps)
    ...
    def train_policy_on_batch(self, batch, use_same_share_obs):
        ...
        if use_same_share_obs:
            cent_obs_batch = _n2t(cent_obs_batch[self.policy_ids[0]])
            cent_nobs_batch = _n2t(cent_nobs_batch[self.policy_ids[0]])
        else:
            choose_agent_id = 0
            cent_obs_batch = _n2t(cent_obs_batch[self.policy_ids[0]][choose_agent_id])
            cent_nobs_batch = _n2t(cent_nobs_batch[self.policy_ids[0]][choose_agent_id])

        dones_env_batch = _n2t(dones_env_batch[self.policy_ids[0]]).to(**self.tpdv)
        ...
        for p_id in self.policy_ids:
            ...
            rewards = _n2t(rew_batch[p_id][0]).to(**self.tpdv)
            curr_obs_batch = _n2t(obs_batch[p_id])
            curr_act_batch = _n2t(act_batch[p_id]).to(**self.tpdv)
            curr_nobs_batch = _n2t(nobs_batch[p_id])
            ...
            if navail_act_batch[p_id] is not None:
                curr_navail_act_batch = _n2t(navail_act_batch[p_id])
            ...
            if isinstance(pol_all_q_out, list):
                ...
                for i in range(len(policy.act_dim)):
                    ...
                    curr_pol_q_out = torch.gather(curr_all_q_out, 1, curr_stacked_act_batch_ind.unsqueeze(dim=-1))
        ...
        if self.use_per:
            ...
            loss = (loss * _n2t(importance_weights).to(**self.tpdv)).mean()
